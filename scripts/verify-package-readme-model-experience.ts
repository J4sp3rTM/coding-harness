...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/host/electron-carrier': { kind: 'none', reason: 'Carries the existing webServer route table over an Electron custom scheme; registers nothing model-facing.' },
  ...
  'packages/client/ui-theme-green': { kind: 'none', reason: 'Browser-side UI plugin layer; registers nothing model-facing.' },
  ...
  'packages/llm/llm-oauth': { kind: 'indirect', reason: 'The seam stores a subscription token; the consuming adapter owns every model-facing use it authorizes.' },
  'packages/llm/llm-oauth-local': { kind: 'indirect', reason: 'The file provider stores subscription tokens; consumers of ctx.llmOAuth own any model-facing behavior.' },
  'packages/llm/command-login': { kind: 'none', reason: 'Both commands execute against the receiving agent without sending anything to the model, and their outcome text is human-only.' },
  ...
  'packages/test-support/harness-eval': { kind: 'none', reason: 'The evaluator submits only the fixture task; selected runtimes own system instructions and tool composition.' },
