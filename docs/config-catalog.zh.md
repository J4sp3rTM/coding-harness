...
- `@deepseek-ai/dsh-command-usage` — 需要 `commands`（[`packages/llm/command-usage/src/index.ts`](../packages/llm/command-usage/src/index.ts)）
...
抽象服务类——部署时应改为加载具体的实现包（参见[能力 seam](capability-seams.md)）。
