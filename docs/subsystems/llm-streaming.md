...
The conversation, streaming, and subscription-authentication types from [`packages/llm`](../../packages/llm/README.md): the `Message`/`ContentBlock` variants every request and durable history share, the fully assembled model request, the raw `StreamChunk` protocol, the adapter contract every adapter must implement, the subscription sign-in service, last-observed provider quota records, and the shared assembler. The [core packages](core.md) hold and log conversation values on every turn; this page declares the shared LLM types.

Source: [`packages/llm/llm/src/types.ts`](../../packages/llm/llm/src/types.ts)

## Subscription authentication

`ctx.llmOAuth` offers provider routes through `LlmOAuthProviderInfo` and exposes only non-secret `LlmOAuthAccount` facts to status surfaces. A sign-in flow talks through `LlmOAuthInteraction`: events report authorization URLs, device codes, progress, and information, while prompts request every value the flow needs. `LlmOAuthTokenStore` is a separate host-only face for the LLM adapter; its serialized `modify` operation lets the provider SDK rotate single-use refresh tokens without exposing them to status or command surfaces. The [sign-in package](../../packages/llm/llm-oauth/README.md) owns the complete provider and storage contract.
...
`ctx.providerStatus` is an ephemeral host-process store of the last rate-limit observation per provider route. Adapters publish parsed headers; `/usage` looks the latest record up by the receiving agent's active route. A missing service, an unavailable record, or a route with no observation contributes no quota line.
