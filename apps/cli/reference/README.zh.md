...
基础组合包挂载原生 DeepSeek 适配器、settings 与凭据提供方、带无密钥 DuckDuckGo 回退的稳定 `web_search`、受防护的 `web_fetch`，以及已禁用的会话遥测。提供方凭据依次从继承环境、`$DSH_HOME/.credentials.yaml`、调用目录的 `.env` 和 `$DSH_HOME/.env` 解析；受管文档从不物化进 `process.env`，而两个 `.env` 文件都是普通启动环境层。搜索优先使用 `DEEPSEEK_API_KEY`（并接受 `DEEPSEEK_SEARCH_BASE_URL`），在该密钥未设置时回退到 DuckDuckGo。
