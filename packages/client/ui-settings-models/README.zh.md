...
模型设置与产品引导插件。同一个 client Cordis 插件会注册 Models 页面和两个有序的首次使用弹窗：版本化内测声明，以及按条件显示的 DeepSeek 官方凭据步骤。两个步骤共用同一套弹窗组件，并继续由 `settings.onboarding` 排序。Models 平面把四个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）、`credentials.describe`（不含值的 configured/source/writable 徽标）与 `llm.models`（按提供方分组的实时模型目录）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。模型目录失败会独立显示，因此成功取得的提供方行仍可使用，同时显示诊断信息。

当 `development-workflow` settings namespace 可用时，Models 页面还会显示「开发代理档位」卡片。T1 用于前沿／例外审查升级，T2 用于实现和大多数委派工作，T3 用于简单、重复、低风险工作；会话或父级模型仍在 composer 中选择，这些路由只作用于委派工作。每个档位都可以继承父级模型，或选择按提供方分组的目录模型以及该准确模型公布的某个推理等级（也可采用提供方默认值）；目录中已不存在的已配置值仍会保留并标为不可用，直到被替换。更换模型会清除之前的推理等级，避免把不兼容值带到另一个模型。卡片暂存三个选择，并以一次带 revision 校验的变更应用（设置 `tiers`，或全部继承时取消设置）；发生冲突时保留草稿并显示冲突，供用户有意重试。只读或缺失 settings 时不能编辑，但目录的部分失败不会禁用仍可用的选择。
...
当 Host 提供 `llm-provider-priority` 时，页面会加入「提供方顺序」卡片，只列出已配置且活跃的路由，并按 Host 的当前生效顺序显示。用户可以拖动行或使用带标签的移动按钮，再携带该分节 revision 保存完整的提供方 ID 列表；发生冲突或写入失败时会保留暂存顺序并报告失败。恢复默认顺序会取消设置 `providers`；未列入已保存列表的提供方仍由 Host 按原有顺序追加。
