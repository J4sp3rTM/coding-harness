...
describe('SearchBlock matches kind', () => {
  it('renders each file as a header group with its matched lines', () => {
    ...
    expect(view.getByText('3 matches · 2 files')).toBeTruthy()
    expect(view.queryByText(/Showing|of/u)).toBeNull()
  })
  ...
  it('folds the pre-cap total into the summary when truncated', () => {
    ...
    expect(view.getByText('Showing 2 of 99 matches · 1 file')).toBeTruthy()
  })

  it('singularizes the match and file units for a single retained result', () => {
    const view = render(<SearchBlock kind="matches" truncated={false} total={1} files={[
      { path: 'a.ts', matches: [{ lineNumber: 3, line: 'const a = 1' }] },
    ]} />)
    expect(view.getByText('1 match · 1 file')).toBeTruthy()
  })
})
...
describe('SearchBlock paths kind', () => {
  it('renders a flat path list with a path-count summary', () => {
    ...
    expect(view.getByText('2 paths')).toBeTruthy()
  ...
  it('folds the pre-cap total into the paths summary when truncated', () => {
    ...
    expect(view.getByText('Showing 2 of 50 paths')).toBeTruthy()
  })

  it('singularizes the path unit for a single discovered path', () => {
    const view = render(<SearchBlock kind="paths" truncated={false} total={1} paths={['src/a.ts']} />)
    expect(view.getByText('1 path')).toBeTruthy()
  })
})
...
describe('SearchBlock empty arm', () => {
  it('shows the placeholder and no copy control for an empty matches result', () => {
    ...
    expect(view.getByText('0 matches · 0 files')).toBeTruthy()
  })
...
describe('SearchBlock height cap', () => {
  ...
  it('slices head and tail over the cap and expands on click', () => {
    ...
    const toggle = view.getByRole('button', { name: 'Expand 6 more result lines' })
    expect(toggle.textContent).toBe('… 6 more lines')
    fireEvent.click(toggle)
    expect(lines(view.container)).toHaveLength(10)
    const collapse = view.getByRole('button', { name: 'Collapse results' })
  ...
  it('counts a file header as one capped row alongside its matches', () => {
    ...
    expect(view.getByRole('button', { name: 'Expand 7 more result lines' })).toBeTruthy()
  })
  ...
  it('renders the head slice alone when the cap leaves no tail', () => {
    ...
    expect(view.getByRole('button', { name: 'Expand 4 more result lines' })).toBeTruthy()
  })
  ...
  it('restores the owning file header above a tail slice that begins mid-file', () => {
    ...
    expect(view.getByRole('button', { name: 'Expand 14 more result lines' })).toBeTruthy()
  })
  ...
  it('caps at the documented default when maxLines is absent', () => {
    ...
    expect(view.getByRole('button', { name: 'Expand 1 more result lines' })).toBeTruthy()
  })
})
