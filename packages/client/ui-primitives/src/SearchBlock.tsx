 ...
 * card's `Showing X of Y`); when it was not capped it is a plain count of what the
...
function summaryText(props: SearchBlockProps, shown: number, truncated: boolean, total: number): string {
  ...
  return props.kind === 'paths'
    ? `${count} ${total === 1 ? 'path' : 'paths'}`
    : `${count} ${total === 1 ? 'match' : 'matches'} · ${props.files.length} ${props.files.length === 1 ? 'file' : 'files'}`
}
          ...
          <div className={css.body}>
            ...
            {hidden > 0 && (
              <button
                ...
                onClick={onToggle}
              >
                {expanded ? 'Collapse' : `… ${hidden} more ${hidden === 1 ? 'line' : 'lines'}`}
              </button>
            )}
