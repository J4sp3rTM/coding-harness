...
After `command.execute` returns a matched command result, this browser emits local `command/executed(sessionId, name, result)`. Other clients receive the durable command nodes through the Host event stream but never this acknowledgment, so a browser-only side effect can select successful results from the client that submitted the command without treating Session replay as an action request. Listener failures are logged and contained one by one; they cannot change the already-admitted command result or prevent later listeners from running. A `/login` result with non-empty text also produces a transient frame-wide toast in the submitting browser through `shell.overlay`; it remains visible while the session is blank and therefore has no command row. The durable row remains the replay and multi-client surface once Chat renders the transcript, while other command results retain their existing presentation.
...
- **Pre-result failures remain session-scoped** — admission and transport failures that occur before a matched result still use the triggering session's composer notice; if that session is already torn down, only the console diagnostic remains.
