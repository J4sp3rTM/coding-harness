...
- **结果产生前的失败仍以会话为作用域**：命令匹配结果产生前发生的准入或传输失败仍使用触发会话的 composer notice；若该会话已经销毁，只剩 console 诊断。
