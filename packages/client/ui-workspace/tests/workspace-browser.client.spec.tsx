...
const t: WorkspaceBrowserProps['t'] = makeTranslate(en, commonEn)
...
describe('WorkspaceBrowser', () => {
  ...
  it('archives a session from the row menu and hides archived rows in both modes', async () => {
    ...
    fireEvent.click(screen.getByText('alpha'))
    fireEvent.click(screen.getByRole('button', { name: 'Session actions for gone-s' }))
  ...
  it('renders a fork child as a top-level row without a session twist', () => {
    ...
    expect(screen.queryByRole('button', { name: /展开|Show less/ })).toBeNull()
  ...
  it('confirms Workspace deletion, explains retention, and blocks duplicate submission', async () => {
    ...
    expect(dialog.textContent).toContain('This removes “Alpha” from the workspace list. The folder and session logs will be kept. Its sessions will appear under Ungrouped.')
    expect(dialog.textContent).toContain('The folder and session logs will be kept. Its sessions will appear under Ungrouped.')
