...
// Standard locale seat stub mirroring the real namespace → common → key chain.
...
describe('workspace browser rows', () => {
  ...
  it('workspace hover card shows its details and copies the full directory path', async () => {
    ...
    try {
      ...
      expect(screen.getByText(/Created \d{4}-\d{1,2}-\d{1,2} /)).toBeTruthy()
  ...
  it('blank New Session rows carry no menu, no time label, and no hover-card time', () => {
    ...
    try {
      ...
      expect(screen.queryByRole('button', { name: /Sessions.*的操作/ })).toBeNull()
