...
The composer Model pane supports implicit case-insensitive filtering over provider and model names and ids: a provider match keeps all of its models, a model match keeps its provider heading, Backspace edits, Arrow keys traverse visible rows, and closing clears the filter. The root pane remains unaffected, no visible input is added, and filtered empty results use localized copy.
