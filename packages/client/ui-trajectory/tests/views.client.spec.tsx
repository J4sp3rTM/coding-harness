...
describe('tab switching in ConversationRoot', () => {
  ...
  it('labels the trajectory tab in the active English locale', async () => {
    const b = await bench()
    const locale = b.ctx.get('locale') as { setLocale(id: string): void }
    locale.setLocale('en')
    const label = tabsOf(b.slots).find(tab => tab.id === 'trajectory')?.label
    expect(label).toBe('Trajectory')
  })
