...
`reportDelivery` 为每条已接受的报告选择父级调度方式。`wakeup`（默认值）在父级空闲时 steer next-step 上下文，让停驻的父级真正开启一个轮次；父级已经在运行时（包括 Stop 之后）则 inject，因为唤醒发送会被改投到后续轮次队列。之所以作为默认值：已经停驻的父级没有别的理由再去查看。报告不是 next-turn 用户提示词，因此绝不会进入后续轮次队列。`quiet` 始终使用 `parent.inject()`，在不启动父级模型请求的情况下添加面向模型的上下文。这是部署调度策略，因此面向模型的 schema 不能在单次调用中选择或覆盖该策略。
...
每次调用都会在执行上报的子级中产生一条简短确认消息。父级还会为上报内容支付 token 成本：唤醒投递会把内容加入父级的下一次请求，并在父级空闲时启动该请求；静默投递只加入内容，不启动请求。
...
仅追加；报告位于父级可复用请求前缀之后。唤醒投递在父级空闲时启动一次父级模型请求，静默投递则不会。
