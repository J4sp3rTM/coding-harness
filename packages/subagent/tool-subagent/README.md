...
Each plugin instance binds one `provider` to one `toolName`; the model receives no provider selector. Load another distinctly named instance to expose another transport. A call may override the child `model` and adapter-owned reasoning `effort` without changing that provider or the instance defaults. The tool registers only while its provider exists, avoiding sibling load-order and provider-reload dependencies. Its description follows `provider.inheritsParentContext`: fresh children require standalone prompts, while forked children already see completed parent turns.
...
| `agentOptions` | Provider-specific child `provider`, `model`, adapter-owned `reasoningEffort`, and positive `maxTokens`; the in-process provider treats explicit values as overrides of inherited parent options. |
...
The generated default [`subagent` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-subagent) under this instance's configured name while its provider exists. Provider context inheritance changes the tool and prompt descriptions. Optional `model` and `effort` fields override this child only; empty values fail before startup, while unknown model ids and adapter-owned effort ids fail loud in the child request path. Enabled background mode adds `run_in_background`, whose default follows `backgroundMode`: `false` for `one-shot`, `true` for `continuable`. While the tool is visible in an assembly's scope, a `tool:<toolName>` system-prompt section tells the model to use the continuable background route by default, start independent delegations together in one assistant message, continue useful work while they run, and choose foreground only when its next action depends on the result; a tool restriction removes both its schema and this guidance.
...
- **Most child policy remains fixed per instance** — calls may override model and effort, but another provider, persona, tool filter, or depth cap requires another distinctly named tool.
