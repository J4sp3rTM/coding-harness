...
| `backgroundMode` | 后台生命周期策略，默认 `one-shot`。调用默认等待结果；显式后台调用分别使用归父级所有的 Task（`one-shot`）或持久化子 agent（`continuable`）。 |
...
当提供方存在时，以当前实例配置的名称公开已生成的默认 [`subagent` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-subagent)。提供方是否继承上下文会改变工具描述和提示词描述。启用后台模式会添加 `run_in_background`，其默认值为 `false`；显式传入 `true` 时选择配置的后台路由。当工具在本次组装的作用域中可见时，一个 `tool:<toolName>` 系统提示词 section 会要求模型默认等待结果，在同一条 assistant 消息中同时启动相互独立的委派以并发执行，并且仅在无需立即使用结果时选择后台；工具限制会同时移除其 schema 和这段指引。
