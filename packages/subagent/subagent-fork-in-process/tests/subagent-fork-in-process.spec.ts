...
describe('dsh-subagent-fork-in-process', () => {
  ...
  it('advertises every start-time capability (depthLimit, outputSchema, toolFilter, persona, agentOptions)', async () => {
    const { ctx } = await setup([])
    expect(ctx.subagents.getProvider('fork')!.capabilities).toEqual({ outputSchema: true, depthLimit: true, toolFilter: true, persona: true, agentOptions: true })
  })
