...
export const FIXTURES: readonly FixtureSpec[] = [
  ...
  },
  {
    id: 'config-layering',
    suite: 'medium',
    task: 'Read CONTRACT.md, then implement immutable layered configuration resolution. Merge plain objects recursively, replace arrays, ignore undefined overrides, validate the resolved runtime fields, and preserve every input object.',
  ...
  },
  {
    id: 'retry-policy',
    suite: 'medium',
    task: 'Read CONTRACT.md, then implement an asynchronous retry policy with injected delay, transient-error classification, abort support, deterministic exponential backoff, and no retry after a permanent failure.',
  ...
  },
  {
    id: 'event-projection',
    suite: 'medium',
    task: 'Read CONTRACT.md, then implement an immutable session-event projector. Enforce sequence continuity, support create/rename/message/delete events, ignore only unknown ignorable events, and reject every other malformed transition.',
  ...
  },
  {
    ...
    suite: 'difficult',
    task: 'Read CONTRACT.md, then complete the versioned key-value store batch API. A batch must validate before mutation, commit atomically against an expected version, publish one immutable change notification after commit, and leave state/version/subscribers untouched on every failure.',
  ...
  },
  {
    ...
    suite: 'difficult',
    task: 'Read CONTRACT.md, then implement the asynchronous dependency scheduler. Validate the entire DAG before execution, honor the concurrency limit, run ready tasks deterministically, block dependents after failure, wait for in-flight tasks, and return a complete status map.',
  ...
  },
  {
    ...
    suite: 'difficult',
    task: 'Read CONTRACT.md, then implement transcript compaction under a token budget. Preserve the system prefix, the latest user turn, assistant/tool-call/tool-result atomic groups, chronological order, and immutability; reject an impossible budget instead of emitting a broken transcript.',
  ...
  },
  {
    ...
    suite: 'stress',
    task: 'Read CONTRACT.md, then repair the asynchronous plugin lifecycle manager. It must validate dependencies, activate concurrently requested plugins once, roll back a failed dependency chain in reverse activation order, keep unrelated active plugins alive, and make shutdown idempotent while reporting all teardown errors.',
  ...
  },
  {
    ...
    suite: 'stress',
    task: 'Read CONTRACT.md, then implement the durable workflow reducer and recovery planner. Replays must enforce monotonic event sequence, deduplicate command keys, recover expired leases, never re-run completed effects, schedule only dependency-ready steps, and produce deterministic commands after any crash point.',
  ...
  },
  {
    ...
    suite: 'stress',
    task: 'Read CONTRACT.md, then repair the multi-tenant tool runtime. Resolve session-scoped tools without cross-tenant cache leakage, enforce permissions before invocation, compose caller cancellation with timeouts, release each acquired resource exactly once, and emit immutable ordered audit records for every outcome.',
