...
- **Crash recovery — preserve valid tail work.** `load` validates every complete compressed frame and scans their decompressed JSONL. If a stale packed chunk row starts before the committed cursor and continues through it, the reader keeps the committed events, discards the row's conflicting raw-chunk prefix, and accepts its contiguous suffix. Exact repeated events are also ignored. A continuation that starts exactly where an interrupted-turn recovery suffix begins supersedes only that deterministic suffix: synthetic tool results, an optional step closer, the interrupted turn closer, and an optional constructor seed boundary. Every other conflicting event or forward sequence gap remains corruption. If the last frame is structurally incomplete, the reader keeps its complete decoded records, truncates from that frame's start, and re-encodes those records with the synthetic tool, step, and turn closers required by the shared [persistence contract](../../../.agents/notes/implemented/architecture/2026-06-14-session-persistence.md). Raw mode truncates from its first incomplete line. An existing compressed artifact with no complete header frame, a checksum/decompression failure in a complete frame, or any other defect at or before the last committed `turn/end` is corruption and rejects.
