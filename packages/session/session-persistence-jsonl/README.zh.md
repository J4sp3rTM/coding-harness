...
- **崩溃恢复：保留有效尾部工作。**`load` 验证每个完整压缩 frame，并扫描解压 JSONL。如果过时的打包 chunk 行从已提交游标之前开始并跨过该游标，读取器会保留已提交事件，丢弃该行中冲突的原始 chunk 前缀，并接纳其连续后缀。精确重复的事件也会被忽略。从中断轮次恢复后缀起点精确开始的续写只会取代该确定性后缀：合成工具结果、可选步骤 closer、中断轮次 closer，以及可选构造函数 seed 边界。其他冲突事件或向前序号缺口仍属于损坏。最后 frame 结构不完整时，读取器保留其完整解码记录，从 frame 开头截断，并使用共享[持久化约定](../../../.agents/notes/implemented/architecture/2026-06-14-session-persistence.md) 需要的合成工具、步骤和轮次 closer 重新编码这些记录。原始 mode 从第一个不完整行截断。已经存在却没有完整 header frame 的压缩工件、完整 frame 中的 checksum/解压失败，或位于最后已提交的 `turn/end` 处或之前的其他缺陷都属于损坏，会被拒绝。
