...
- **Codex plan headers require SSE** — pi-ai exposes `x-codex-*` plan headers only on its SSE transport. The default WebSocket transport produces no HTTP response, so no Codex plan window can be observed there; the adapter does not change that default.
