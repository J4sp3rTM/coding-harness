...
When the optional [`ctx.providerStatus`](../provider-status/README.md) service is composed, the adapter publishes what each successful response's documented headers disclose about that route's quota. OpenAI's `x-ratelimit-*` and Anthropic's `anthropic-ratelimit-*` fields become per-dimension limit/remaining pairs, while Anthropic OAuth unified utilization and OpenAI Codex OAuth used-percent fields become subscription plan windows with provider reset timestamps. Every other header is ignored; recognized fields whose values all fail to parse record an explicit unavailable state, and a response with no recognizable fields records nothing. An empty axis is omitted from the publication so a later harvest can keep the other axis. The adapter also registers `providerStatus.refresh`: for `openai-codex` it GETs the Codex OAuth usage endpoint `chatgpt.com/backend-api/wham/usage` and publishes its rate-limit windows, while `xai` GETs SuperGrok `cli-chat-proxy.grok.com/v1/billing?format=credits` and publishes `creditUsagePercent` as a `7d`/`30d` plan window. These HTTP observations are advisory bookkeeping: an observer or harvest failure can never fail the model call or `/usage`, and without the service composed nothing changes.
...
- **Empty-route startup is debug, not stderr** — `reportRoutes()` logs the ordinary first-run emptiness (`subscription sign-ins: (none)`, no registered routes) at debug so stderr stays the error channel. Configured routes log at info; a signed-in subscription that serves no models logs at warn.
...
- **Codex plan windows use a provider endpoint** — the default WebSocket transport produces no HTTP response headers, so `/usage` reads `chatgpt.com/backend-api/wham/usage` with the stored Codex OAuth token. This is separate from ordinary model transport and does not send a model prompt.
