...
一个服务插件，通过 agent loop（智能体循环）在已关闭步骤上触发的 `agent/request-error` waterfall（瀑布式事件）应用确切提供方重试策略。它不包装 `ctx.llm.stream()`：每次适配器调用仍是一次提供方尝试，每次重试都保留在相同的编号轮次与步骤中。

其他能力可以通过 `ctx.llmRetry.register()` 注册进程本地贡献。贡献可以为符合条件的请求向每个由提供方所有的等待增加取消信号，也可以提供一项回退策略。有限提供方策略先运行；其预算耗尽或错误码不符合后，先运行下游 waterfall 恢复，再由恰好一项贡献回退接管请求。提供方 `always` 策略仍会先委托下游 waterfall 恢复，并继续作为重试所有者；此时不会查询贡献回退。多项同时符合条件的回退会明确失败，不依赖注册顺序。退避、重试编号、持久重试事件与 teardown 排空仍只由本服务负责。
...
等待前，插件会追加一条不进入表层的 `llm/retry` 事件，其中包含共享 `retryId`、提供方、mode、已解析策略的规范 key、失败和计划延迟。该载荷由可安全用于浏览器的 `@deepseek-ai/dsh-llm-retry/types` 子路径导出，因此远程渲染器无需加载策略运行时即可使用该持久状态。提供方 key 包含所有影响行为的字段；贡献 key 还包含贡献者 id。normal mode 的 code 会排序，因为合格性采用集合成员关系判断。只有提供方与完整策略 key 都相同的事件才会延续重试编号；因此，替换路由或采用不同贡献后会开始独立历史。normal 事件包含有限上限；always 事件省略该上限，UI 会渲染 `∞`。等待完成时，插件会在返回 `{ kind: 'retry' }` 前立即追加 `llm/retry-started`，其中带有相同的 `retryId`、轮次、步骤与重试编号；退避期间取消则不会写入 started 事件。取消与插件 dispose 会中止活跃退避，在应用中止前等待活跃的委托恢复结算，并使 dispose 前捕获的 callback 只能以失败结束。

单独发布的 `./invariant` 配套模块会检查每个已调度重试是否指向当前开启轮次及其最新已关闭步骤，是否与失败请求的持久提供方匹配，是否携带非空的提供方与策略标识，是否满足 mode 特定边界，是否拥有唯一步骤记录和正确的策略重试编号，以及是否携带有界定时器延迟。它还要求每个 `llm/retry-started` 事件通过相同的 `retryId`、轮次、步骤与重试编号指向一个先前调度的尝试，并拒绝重复的 started 事件。full jitter 可以在下界调度为零毫秒。
...
- **有限预算可叠加**：normal mode 只统计已配置 code 与确切策略 key，上下文溢出压缩（compaction）和贡献回退则拥有独立预算。
- **恢复策略显式组合**：always mode 会先接受下游 waterfall 重试，再应用自身策略。有限提供方策略先于贡献回退，多项符合条件的贡献会被拒绝。下游策略如果忽略取消且永不结算，仍会阻止回退、轮次完全停稳和插件 dispose 完成。
