...
缺字段的条目读作未登录而不是失败：被截断或手工编辑过的条目由一次登录修复，另一种选择是让一个形似令牌的值抵达供应商请求。对于签发不可刷新访问授权的供应商，空刷新令牌是有效值。
...
登录会存下一个令牌组；随后 pi-ai 适配器会激活尚未配置的目录路由、公开其模型、使用订阅认证，并在该存储自己的锁下轮换可刷新令牌。若 `llm-pi-ai` 设置分节中存在 profile，则它仍具权威性；`auth` 字段以及「已存登录」与「已配置 key」之间的优先级归[那个包的 README](../llm-pi-ai/README.md) 所有。
