...
- `registerRefresh(fn)` installs the single on-demand harvest; a later registration replaces it and the disposer forgets only the function it installed. `refresh(routeId, signal)` calls that harvest or is a no-op. An omitted `dimensions` or `windows` field on `recordSnapshot` keeps the previous snapshot's values for that axis so a header observation and a billing harvest can share one record.
...
Publishing to and reading from `ctx.providerStatus` performs no provider call. `refresh` may, when an adapter registered a harvest: that probe is owned by the adapter, not this store.
...
- **Passive headers plus optional harvest** — adapters publish what a response already carried, and `/usage` may ask `refresh` to harvest once. Providers with neither headers nor a registered harvest stay unobserved.
