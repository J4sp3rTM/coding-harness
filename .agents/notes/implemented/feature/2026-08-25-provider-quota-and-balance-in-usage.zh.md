...
- **通过 `ctx.get` 保持可选。** 两个消费者都没有把服务声明为注入。未挂载它们的组合只报告会话自身的数字。随附的基础 bundle 挂载 `dsh-provider-status`，以便 `/usage` 能报告最近观察到的配额；省略该行的自定义组合仍保持不变。
...
**在命令处理器里实时拉取配额。** 否决：配额免费地随会话已经在发的响应而来，轮询只会为展示增加提供方调用；只有余额端点缺少被动来源，因此才按需获取。

## Consequences

随附的基础组合无需离开会话即可看到发布限流头路由的配额百分比与重置倒计时、Anthropic OAuth 与 Codex OAuth 的订阅套餐余量，以及 DeepSeek 的剩余美元余额。省略该存储或 DeepSeek 适配器的组合看不到任何变化。Codex 的套餐窗口只在 pi-ai 的 SSE 传输上可见，因为其默认的 WebSocket 传输不产生可读的 HTTP 响应。代价是 llm 组多出的一个小包、命令处理器里的两个可选读取，以及一条长期义务：保持归一化器白名单封闭——新提供方的标头在被文档化并加入白名单之前保持不可见。
