...
The shared base mounts `dsh-web` with the ordered preference `searchProviders: ['deepseek-official', 'duckduckgo']`, `dsh-web-search-deepseek`, the keyless [`dsh-web-search-duckduckgo`](../../../../packages/web/web-search-duckduckgo/README.md), `dsh-web-fetch-http`, and `dsh-tool-web` with `searchTimeoutMs: 60000` — [the preference-list Agent Note](../architecture/2026-08-25-web-provider-preference-list.md) owns that change from this decision's original pinned `searchProvider: deepseek-official` with fetch disabled, and [the private-network guard Agent Note](../architecture/2026-08-25-web-fetch-private-network-guard.md) owns enabling `web_fetch`. The shared base makes search (and now guarded page retrieval) defaults for TUI, browser, and headless sessions. The ordered list keeps selection independent of registration order and leaves personal or `--config` overlays able to replace or disable the rows; a single `$DSH_WEB_SEARCH_PROVIDER` pin still forces one route. The one-minute shipped budget covers an auxiliary DeepSeek Messages request plus server-side retrieval while leaving `dsh-tool-web`'s provider-neutral 30-second default unchanged for custom compositions.
...
**Enable search and fetch together.** Rejected at the time, because default `web_fetch` would allow model-selected anonymous outbound HTTP(S) retrieval to arbitrary URLs with no private-network guard. [The guard Agent Note](../architecture/2026-08-25-web-fetch-private-network-guard.md) later supplied that guard and enabled fetch; this alternative is recorded as rejected-under-then-current-conditions, not as permanently declined.

## Consequences

Native model requests on every shipped surface carry the `web_search` schema and search-first prompt guidance; Web/headless Code Mode exposes the same capability beneath `run_code`. Under this decision the prompt told the model to use returned snippets and never advertised a disabled `web_fetch`; since [the guard landed](../architecture/2026-08-25-web-fetch-private-network-guard.md), `web_fetch` is advertised too. Search adds a complete auxiliary model call and may use the native server tool multiple times; its exact secret-free request remains reconstructable from the initiating session log. Under this decision the default offered search-result snippets and source metadata but no page retrieval; deployments needing full-page fetch had to opt in until [the guard Agent Note](../architecture/2026-08-25-web-fetch-private-network-guard.md) changed that. The Web snapshot lane boots the shipped tree, drives a replayed `web_search` call through the real DeepSeek provider against a local Messages fixture, asserts the durable auxiliary request and structured result, and pins the settled browser presentation. The TUI/Web composition smokes pinned the shared `web_search` roster and absence of `web_fetch` while that held (the rosters now include it); the built composition dump pins the one-minute shipped search budget; provider tests pin missing, stored, and rotated credential behavior plus literal and ambient compatibility.
